module.exports = {
  ...
  button: ["deposit_"],
  func: async (context, { vk, _user, gamepodnad, pod7nad, db, util, _cmpen }) => {
    let _cmd = pod7nad.pod7nad.split('_');
    let _realgame = await gamepodnad.realGame(context.peerId)
    let _thisGame;
    if (_realgame.newgamew == false) _thisGame = await gamepodnad.getGame(context.peerId);

    let _name = { 'up': `больше`, 'down': `меньше`, 'number': `число` };
    let _num = 7; let _coin = 0;
    ...
    if (_user.bonus < _user.balance) {
        ...
        _thisGame.users.forEach((res, i) => {
          res.inGame.forEach((g, k) => {
            if (g.peer_id == Number(context.peerId)) {
              if (g.type != _cmd[1] && res.name == _user.name && _cmd[1] != `number`) return legeng = true
            }
          })
        });
      ...
      if (_realgame.newgamew == true) {
        _thisGame = await gamepodnad.getGame(context.peerId);
